...
class FunCog(commands.Cog):
    ...
    async def waterboard(self, ctx) -> None:
        # Drags a user between calls randomly and returns them to the original one
    ...
    async def image(self, ctx):
        ...
        if path == 403:
            await ctx.send(embed=discord.Embed(title="**ERORR**", description="We have hit the rate limit for "
                                                                              "the month"))
            return 0
        if path == -1:
    ...
    @commands.command(brief='Shows the astronomy photo of the day', description='Shows the astronomy photo of the '
                                                                                'day, with a descrption of what '
                                                                                'it is. Click on the title for '
                                                                                'the full image')
    async def APOD(self, ctx, date=""):
        ...
        explanationTrunc = (stor['explanation']) if len(stor['explanation']) < 1024 else stor['explanation'][
                                                                                         :1021] + "..."
    ...
    async def recap(self,ctx):
        ...
        culprit = ctx.message.mentions[0]
        counter = 0
        hard_counter = 0
        async for message in ctx.channel.history():
            if "nigg" in message.content.lower():
                counter+=1
                if "er" in message.content.lower():
                    hard_counter+=1
        embed = discord.Embed(title = "Search Results",description=f"{culprit.name} has said the N-word `{counter}` times \nThe Hard-R `{hard_counter}` times" ,color=discord.Color.dark_red())
